...
impl DbConnection {
    pub async fn new(db_path: &PathBuf) -> Result<Self> {
        let db_exists = DbConnection::check_if_database_exists(db_path);
        if !db_exists {
            return Err(anyhow::anyhow!("Database file does not exist."));
        }

        let pool = DbConnection::create_connection_pool(db_path).await?;
    ...
    async fn create_connection_pool(db_path: &PathBuf) -> Result<Pool<Sqlite>> {
    ...
    pub async fn enable_sqlite_foreign_key_support(&self) -> Result<()> {
